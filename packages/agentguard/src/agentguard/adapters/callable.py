"""Callable Python function agent adapter."""
...
AgentFn = Callable[[list[str], ToolBox, RunContext, ModelProxy, dict[str, Any]], str]
    ...
    def run(
        ...
        *,
        ...
        run = Run(
            ...
            metadata={"agent_type": "callable", **self.config},
        )
        trace = RunContext(run)
        trace.mark_started()
        toolbox.context.update(fixtures or {})
        ...
        try:
            output = self.fn(user_messages, toolbox, trace, proxy, fixtures or {})
